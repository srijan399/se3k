...
function seed(store) {
    ...
    // ---- Decisions ----
    const pgbouncer = store.upsertDecision('adopt-pgbouncer', 'Adopt PgBouncer connection pooling for the checkout service');
    const optimisticCart = store.upsertDecision('optimistic-cart', 'Ship optimistic cart updates with a rollback toast + retry');
