...
function seed(store) {
    ...
    // ---- Decision ----
    const decision = store.upsertDecision('drop-redis-ratelimit', 'Drop the Redis-backed rate limiter in favor of an in-process token bucket');
    ...
    store.addEdge('MADE_CALL', mia.id, decision.id, daysAgo(8), {
        ...ch,
        ts: daysAgo(8),
        excerpt: 'Decided per-pod buckets are fine: limits are advisory and Redis latency was the real outage cause.',
    });
    store.addEdge('RELATES_TO', decision.id, rl.id, daysAgo(8));
    store.save();
    const snap = store.snapshot();
    console.log(`Seeded graph: ${snap.nodes.length} nodes, ${snap.edges.length} edges → ${''}`);
}
